...
namespace ModManager
{
    public class ModInfo
    {
        /*
         ...
         -80
        */
        private static string[] exceptions = new string[8];
        ...
        {
            mainButton.Disable(state);
        }
        public void SetActive(bool value)
        {
            if (pdbButton != null) pdbButton.gameObject.SetActive(value);
            mainButton.gameObject.SetActive(value);
            if (IsException) mainButton.gameObject.SetActive(false);
            if (value)
            {
                if (PluginInfo != null)
                {
                    modManager.modInfo.text = "GUID: " + PluginInfo.Metadata.GUID + "\nVersion: " + PluginInfo.Metadata.Version;
                }
                else modManager.modInfo.text = "";
                modManager.modName.text = ModManager.InsertNewLineEveryNCharacters("Mod name: " + name, 25);
            }
        }
        ...
        {
            if (mainButton.Value || !mainButton.gameObject.activeSelf) return name + dllEnabled;
        ...
        public ModInfo(string file) 
        {
            ...
            mainButton = modManager.FixedCreateToggle("ModActive", "Active", GetDLLState(name) && !exceptions.Contains(name), pos, 100, "Is mod active");
